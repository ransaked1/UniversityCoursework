...
public class ChatWindow {
    ...
    public ChatWindow(App app, Communicator communicator) {
        ...
        //Add a basic sidebar
        var sideBar = new UserList();
        pane.setRight(sideBar);
        var changeUsername = new TextField();
        var muteSound = new CheckBox("Enable Sound");
        sideBar.getChildren().addAll(changeUsername,muteSound);

        //Link the UI elements to the corresponding properties
        changeUsername.textProperty().bindBidirectional(app.usernameProperty());
        muteSound.selectedProperty().bindBidirectional(Utility.audioEnabledProperty());
